...
* В третьем блоке 
    1. исследовал  данные;
    2.  попробовал найти закономерности, позволяющие сформулировать предварительные гипотезы относительно того, какие факторы являются решающими для оформления депозита;
...
* В пятом блоке 
    1. Для начала реализовал самый простой алгоритм — логистическую регрессию.В качестве алгоритма оптимизации  использовал: 
    * 'sag',  
    * random_state возьмём число 42 
    * и запустим 1000 итераций.
* В шестом блоке 
    1. Полученные знания использовал для  объединения их в случайный лес.
...
В четвертом блоке: 
* Преобразование категориальных переменных в числовые форматы, такие как one-hot encoding или label encoding, делает их пригодными для использования в алгоритмах машинного обучения.
* Масштабирование данных с помощью StandardScaler или MinMaxScaler приводит все признаки к одному масштабу, что улучшает производительность многих алгоритмов.
...
В шестом блоке: 
* Ансамбли моделей, такие как случайные леса и градиентный бустинг, обычно дают более высокую точность по сравнению с одиночными моделями за счет объединения предсказаний нескольких слабых моделей.
* Использование ансамблей помогает снизить переобучение и улучшить обобщающую способность моделей.

Работа проделана большая, данные можно использовать для будущих маркетинговых исследований. 
